...
int main(void) {
    ...
    if (pid == 0) {
        char *argv[] = {(char *)"/bin/test/fs", 0};
        sc3(SYSCALL_EXEC, (uintptr_t)"/bin/test/fs", (uintptr_t)argv, 0);
        sc1(SYSCALL_EXIT, 127);
    }
    if ((int64_t)pid < 0) fail("fork for exec succeeds");
    else {
        int status = -1;
        int64_t waited = (int64_t)sc1(SYSCALL_WAIT, (uintptr_t)&status);
        expect("wait returns exec child pid", waited == (int64_t)pid);
        expect("exec child exits cleanly", status == 0);
    }
    if (fails) { out("process: FAIL "); dec((uint64_t)fails); out(" failure(s)\n"); return 1; }
    out("process: PASS\n");
    return 0;
}
