...
int main(int, char **) {
    ...
    if (fd < 0) {
        out(STDERR, "shutdown: cannot open /dev/power\n");
        return 1;
    }

    out(STDOUT, "shutdown: writing 'poweroff' to /dev/power\n");
    if (write_all(fd, "poweroff\n") != 0) {
        out(STDERR, "shutdown: /dev/power rejected request\n");
        syscall1(SYSCALL_CLOSE, (uint64_t)fd);
        return 1;
    }

    syscall1(SYSCALL_CLOSE, (uint64_t)fd);
    out(STDERR, "shutdown: request returned without powering off\n");
    return 1;
}
