...
namespace elf {
    static constexpr uint64_t USER_TOP = 0x0000800000000000ULL;
    ...
    elf_info load(const char *path) {
        ...
        if (image_start == 0 || image_end <= image_start || image_end >= USER_TOP) {
            return fail();
        }
        ...
        if (heap_start <= image_end || heap_start >= USER_TOP) {
            vmm::destroy_user_address_space(user_pml4);
            return fail();
        }
